...
Loading states use a compact 16px monochrome spinner placed inline with the loading message. Keep the spinner close to the text with a small gap so the state reads as one unit rather than a separate decorative element.
